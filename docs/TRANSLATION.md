...
- `header`/`footer`/`figure`/`equation`/`reference` —— 直接跳过，不进入 policy
- `figure_caption`/`table_caption` —— 剥离 `Figure 2.` / `表 3` 编号，只翻译后半
- `table` + `content_format=html` —— 解析 HTML，每个 `<td>/<th>` 独立走完整个 policy → cache → model → quality gate 管线，保留 `colspan`/`rowspan`
- 其他 —— 按 `content_format` 走 text 或 markdown 分支

**每个表格 cell 都会独立命中 policy**：数字/单位/URL 等自动 skip，短英文经过 dictionary 或送模型。所以 skip 规则修改不当会**同时**影响 /translate 和表格里的单元格判定。加规则前先想好这两类场景的表现。
