...
/`ocr/translate` 是构建在这套 policy + cache + quality gate 上的**类型感知批量接口**。它接受调用方声明的元素类型（PP-Structure 风格：text/title/table/header/footer/caption/equation/figure/reference…），按类型分发：

- `header`/`footer`/`figure`/`equation` —— 直接跳过，不进入 policy
- `reference` —— 只翻引号内的文章标题；作者/期刊名/DOI 保留原样
- `figure_caption`/`table_caption` —— 剥离 `Figure 2.` / `表 3` 编号，只翻译后半
- `table` + `content_format=html` —— 解析 HTML，每个 `<td>/<th>` 独立走完整个 policy → cache → model → quality gate 管线，保留 `colspan`/`rowspan` 与内联标签（sub/sup/b/i/a…）
- 所有类型（表格 cell 除外）在送模型前都会做**语言检测**：如果 content 主导脚本已经是 target 语言，直接跳过

**表格 cell 走保守模式**（`TranslateConservative`）：不在字典里的短英文单词（≤12 字符）自动 skip 保留原文。避免姓名列/州名缩写被误翻。**加 skip 规则前记得这两条通道**（普通 /translate 和表格 cell）行为可能不同。

详细 API、字段、reason 枚举、局限性见 [`OCR.md`](./OCR.md)。
